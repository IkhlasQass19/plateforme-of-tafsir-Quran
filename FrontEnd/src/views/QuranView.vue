      ...
      <div class="row">
        ...
        <div class="col-md-4">
          <select id="tafsir-selector" class="form-select w-75 m-auto mb-md-0 mb-2 main-color-one shadow-none"
            ...
            <option v-for="tafsir in tafsirs" :value="tafsir.ntafssir">
              {{ tafsir.nom }}
            </option>
          </select>
        </div>
      ...
      <div class="row">
        <div class="col-lg-5 col-md-6">
          <audio id="player" class="d-block w-100 mb-2" :current-ayah="currentAyah"
            @playing="scrollToActiveAyah(currentAyah)" @ended="switchAudio" controls>
    ...
    <div id="body" class="mt-3">
      ...
      <div class="ayah-box ketab-font d-inline" v-for="ayah in ayahs" v-if="!fetchSurahFailed && !loading"
        ...
        <p class="ayah-text d-inline main-color-one" @click="playAyahAudio(ayah.number)">
          {{
            surahNo !== 1 && ayah.numberInSurah === 1 ? ayah.text.replace('بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ ', '') :
            ayah.text
          }}
        </p>
        ...
        <div class="ayah-additional-data" v-if="!withoutTafsirs">
          <template v-if="ayahsTafsir.length === 0">
            <span class="additional-data-title tafsir-name">{{ selectedTafsirName }}</span>
            <span class="additional-data-content tafsir-content">هذا المفسر لم يفسر هاته الاية</span>
          </template>
          <template v-else>
            <template  v-if="checkexist(ayahsTafsir,ayah.numberInSurah)" v-for="ayahTafsir in ayahsTafsir">
              <span class="additional-data-title tafsir-name" v-if="ayahTafsir.naya===ayah.numberInSurah">{{ selectedTafsirName }}</span>
              <span class="additional-data-content tafsir-content" v-if="ayahTafsir.naya===ayah.numberInSurah">{{ ayahTafsir.text }}</span>
            </template>
           <template v-else>
              <span class="additional-data-title tafsir-name">{{ selectedTafsirName }}</span>
              <span class="additional-data-content tafsir-content">هذا المفسر لم يفسر هاته الاية</span>
...
export default {
  ...
  methods: {
    ...
    //check exist
    checkexist(ayahsTafsir,numberInSurah){
      console.log(ayahsTafsir.some(ayahTafsir => ayahTafsir.naya===numberInSurah));
      return ayahsTafsir.some(ayahTafsir => ayahTafsir.naya===numberInSurah);
    },
    //get asma2 mufassirin
    getTafsirs() {
      this.axios.get('http://localhost:8080/mufassirun')
        .then(response => {
          const tafsirs = response.data;
          if (response.status === 200) {
            // Assign the retrieved data to the "tafsirs" variable
            this.tafsirs = tafsirs;
            console.log("tafssir asmae:");
    ...
    getAyahsTafsir(tafsirId, tafsirName) {
      ...
      let surahNo = this.$route.params.id.length > 0 ? this.$route.params.id : 1
      this.axios.get(`http://localhost:8080/surah/${surahNo}/${tafsirId}`)
        .then(response => {
          this.selectedTafsirName = tafsirName;
          if(response.data.length==0)
            this.ayahsTafsir =[];
          else
          this.ayahsTafsir = response.data;
